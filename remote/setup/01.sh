...
# Prompt to enter a password for the PostgreSQL clinic user (rather than hard-coding
# a password in this script).
read -p "Enter password for clinic DB user: " DB_PASSWORD
...
# Set up the clinic DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE clinic"
sudo -i -u postgres psql -d clinic -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d clinic -c "CREATE ROLE clinic WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the clinic database to the system-wide environment 
# variables in the /etc/environment file.
echo "CLINIC_DB_DSN='postgres://clinic:${DB_PASSWORD}@localhost/clinic'" >> /etc/environment
